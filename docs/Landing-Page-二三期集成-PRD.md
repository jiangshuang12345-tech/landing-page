...
## 7. 核心模块五：韩国 KOL 渠道定制化交互 (新增)

针对韩国 (KR) 的 KOL 渠道，首页 (Landing Page) 采用区别于标准模板的特殊定制展示，具体表现为**“图文与表单合一”**的模式（具体样式与交互可见项目原型的 KR-KOL 展示区）。

### 7.1 视觉展现层
* **长图背景模式**：改变原有上下结构的短平快注册页，将「品牌介绍详情 / KOL 背书内容」直接作为极长的主视觉背景图渲染。
* **悬浮注册框**：原有的注册登录表单（包含手机号、验证码、获取按钮等）不再居中独占屏幕，而是作为透明卡片悬浮（或固定 Top 值绝对定位）在长图的特定区域上（如距离顶部 300px 处）。

### 7.2 动态吸底唤起 (Sticky Button)
由于图片较长，用户在下滑阅读品牌详情时，注册表单会被移出屏幕可视区。需实现以下引导交互：
* **监听滚屏离开 (Intersection Observer)**：当前端检测到上方的「注册登录卡片」完全离开视口 (Viewport) 时，页面底部滑出（或渐显）一个固定的悬浮引导按钮（例如：`지금 Dino English 시작하기` / 现在开始 Dino English）。
* **点击回滚 (Smooth Scroll)**：当用户点击该底部吸底按钮时，页面平滑滚动 (Smooth Scroll) 自动回到顶部注册表单区域，引导完成注册。
* **状态重置**：当用户滚回顶部、注册表单重新进入视口时，底部的引导按钮自动隐藏。

> **后续购买流**：用户在上述定制化首页完成登录后，直接跳转至套餐选择页（`plans`），后续的购买、优惠码、支付流程与该 KOL 渠道码所绑定的常规链路完全一致。
