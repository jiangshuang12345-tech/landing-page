...
- 不改一期第三方 OAuth / 退款公式 / VIP 叠加规则（但马来西亚登录方式有专项调整）
...
| redeem_limit | int / null | 可兑换次数上限；null = 不限（一期「一码多人」），也可配置为具体数字（如 1、5 等） | `1` |
...
4. **【新增】关联校验与统计**：
   - C 端输入兑换码时，必须匹配当前选中的 SKU，否则提示 "Wrong coupon code, retry"。
   - 支付成功时，需要增加该优惠码的 `redeem_count`（兑换次数），并在订单中绑定核销的优惠码，便于后续按销售或 KOL 统计分发和使用绩效。
...
> **业务规范提醒**：由于沙特和韩国存在「常规媒体渠道（无优惠）」与「KOL 渠道（有优惠）」的差异，为了便于区分，同周期的 SKU 需要分别创建两套，并通过**名称后缀**进行区分（如：`1-week` vs `1-week-KOL`）。

| 国家 | SKU ID | 名称 | 原价 | Best Value | 券 ID | 折扣类型 | 力度 | 优惠码 | 使用次数限制 |
|---|---|---|---|---|---|---|---|---|---|
| 沙特 | 7824 | 1-week | SAR 51.99 | — | - | - | - | - | - |
| 沙特 | 7825 | 1-month | SAR 174.99 | — | - | - | - | - | - |
| 沙特 | 7826 | 1-year | SAR 1099.99 | ✓ | - | - | - | - | - |
| 沙特 | 7827 | 1-week-KOL | SAR 51.99 | — | 6666 | 百分比 | 10% | 可生成多个 `DINO****` | 1次 / 5次 / 无限制 |
| | | | | | 6667 | 立减 | 20 | 同上 | 1次 |
| 沙特 | 7828 | 1-month-KOL | SAR 174.99 | — | 6670 | 百分比 | 10% | | 1次 |
| 沙特 | 7829 | 1-year-KOL | SAR 1099.99 | ✓ | 6671 | 百分比 | 20% | | 1次 |
| 韩国 | 7830–7832 | 1-week/month/year | KRW … | 年包 ✓ | - | - | - | - | - |
| 韩国 | 7833–7835 | 1-week/month/year-KOL | KRW … | 年包 ✓ | 6672–6674 | 百分比 | 10%/10%/20% | | 1次 / 5次 |
| 马来 | 7836–7838 | 1-week/1-month/1-quarter | RM … | 月包 ✓ | 6675–6676 | 百分比 | 40%/45% | | 无限制 |
| 印尼 | 7839–7840 | 1-month/1-year | IDR … | 月包 ✓ | 6677–6678 | 百分比 | 40%/45% | | 无限制 |

#### 渠道 × SKU × LP

| 国家 | 渠道 | 渠道码 | 绑定 SKU | LP 说明 |
|---|---|---|---|---|
| 沙特 | Meta Ads | `Zr7Gx4d` | 7824, 7825, 7826 | LP-1：常规媒体货盘（无优惠版） |
| 沙特 | KOL 渠道 | `Xb3Mq6s` | 7827, 7828, 7829 | LP-2：KOL 专属货盘（可绑券版） |
| 沙特 | Google Ads | `Uw5Tn8f` | 7824 | LP-3：仅投放常规周包 |
| 韩国 | 常规媒体 | … | 7830, 7831, 7832 | 默认标准完整货盘 |
| 韩国 | KOL 渠道 | … | 7833, 7834, 7835 | KOL 专属完整货盘 |
| 马来 | Meta / KOL | … | 7836, 7837 | 侧重周/月包 |
| 马来 | Google / 商店 | … | 7838 | 侧重季包 |
| 印尼 | Meta | … | 7839, 7840 | 双 SKU |
| 印尼 | 其他渠道 | … | 7839 或 7840 | 单 SKU 投放 |
...
基于最新的交互原型，马来西亚（MYR）市场在登录及收银台环节进行以下调整。

#### 6.6.1 登录方式精简 (马来专属)
为优化转化漏斗并符合当地业务策略，**马来西亚 (MY) 的 Landing Page 仅保留「手机号注册/登录」方式**：
- 前端需隐藏或移除所有第三方快捷登录方式（如 Google, Apple, Facebook 等）。
- 仅展示国家区号（默认 `+60`）、手机号输入框以及获取验证码（Get Code）模块。
...
| 优惠码校验失败（含跨SKU误用） | 统一 toast 提示错误（如：`Incorrect promotion code` 或 `Wrong coupon code, retry`） |
...
- [ ] 一期默认渠道码仍可完成完整购买
- [ ] 支付方式、VIP 发放、退款规则与一期一致（除新增支付外）
- [ ] **登录方式检查**：马来西亚 (MY) 落地页仅保留手机号输入框与获取验证码登录，不得展示 Google/Apple/Facebook 等第三方快捷登录选项；其他国家不受影响。
...
| 模块 | 一期 | 二期 |
|---|---|---|
| 注册 / 登录 | ✅ 完整定义 | **马来西亚仅保留手机号注册/登录**，其他国家不变 |
| 支付方式 | ✅ 按国 | 不变 |
| 支付成功 / 复访 | ✅ | 不变 |
| 退款 | ✅ | 不变 |
